...
const METRIC: &str = paired_lock_protocol::METRICS[0];
...
#[inline]
fn baseline(file: &File) -> io::Result<()> {
    fs2_baseline::FileExt::lock_exclusive(file)?;
    fs2_baseline::FileExt::unlock(file)
}

#[inline]
fn candidate(file: &File) -> io::Result<()> {
    fs2_candidate::FileExt::lock_exclusive(file)?;
    fs2_candidate::FileExt::unlock(file)
}
...
fn measure(
    ...
    match comparison {
        Comparison::Ab => {
            measure_subjects(file, baseline, candidate, sample_size, warm_up, measurement)
        }
        Comparison::Aa => {
            measure_subjects(file, baseline, baseline, sample_size, warm_up, measurement)
        }
    }
}
