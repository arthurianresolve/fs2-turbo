...
compatibility checks, workflow policy, and native command execution. Typed
configuration models reject unknown fields and carry explicit schema versions.
The command runner retains stdout and stderr handles, enforces bounded process
lifetimes, and preserves native status and cleanup failures. Windows capture
directories retain protected ancestry and private DACLs. Process containment
provides lifecycle cleanup, not an adversarial sandbox. The current checkout
and host toolchain are trusted inputs. Tooling dependencies do not enter the
